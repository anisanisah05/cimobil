...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_barang', $this->input->post('kode_barang')[$i])->get('mobil')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_barang', $this->input->post('kode_barang')[$i])->update('mobil', $u_stok);
			}
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('mobil', 'mobil.kode_barang = detail_transaksi.kode_barang')
